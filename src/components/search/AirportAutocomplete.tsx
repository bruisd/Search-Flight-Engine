...
import { useAirportSearch, useDebounce } from "../../hooks";
...
// Hardcoded airport list (fallback for when API is unavailable)
...
function AirportAutocomplete({
  ...
  const {
    ...
    isLoading,
    error,
  } = useAirportSearch(debouncedInputValue);
  ...
  // Use API results if available, otherwise filter fallback list
  const airports = useMemo(() => {
    // If we have API results, use them
    if (apiAirports.length > 0) {
      return apiAirports;
    }

    // If no input or input is too short, return empty array
    if (debouncedInputValue.trim().length < 2) {
      return [];
    }

    // Fallback to local filtering if API returned no results
  ...
  }, [apiAirports, debouncedInputValue]);
      ...
      <Autocomplete
        ...
        renderOption={(props, option) => (
          <Box
            ...
            <Box
              sx={{ fontSize: "0.75rem", color: "#616f89", marginTop: "2px" }}>
              {option.city}, {option.country}
            </Box>
          </Box>
        )}
